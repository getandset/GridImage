...
public class ImageCache {
 ...
 // default memory cache size in kiloByte
    private static final int DEFAULT_MEN_CACHE = 4 * 1024; 
 // default disk cache size
    private static final int DEFAULT_DISK_CACHE = 10 * 1024 * 1024; 
    ...
    /**
     * this constructor can't be instance directly,use getInstance to the object
     * of this
     */
    ...
    public static ImageCache getInstance(FragmentManager fm,
    ...
    private static RetainFragment findOrCreateFramgment(
     ...
     * 
     * @param key
     *            identifier for a bitmap to store in cache
     * @param value
     *            bitmap to be stored in cache
     */
	...
	synchronized (mDiskCacheLock) {
		...
		try {
			...
			if (null != editor) {
			    ...
			    value.getBitmap().compress(DEFAULT_COMPRESS,
				    PICTURE_QUALITY, out);
			    editor.commit();
			}
		    } else {
	...
	synchronized (mDiskCacheLock) {
		...
		try {
		    ...
		    if (null != snapshot) {
			InputStream inputStream = snapshot
				.getInputStream(DISK_INDEX);
			if (null != inputStream) {
	...
	return candidate.getWidth()==width/options.inSampleSize&&
	       candidate.getHeight()==height/options.inSampleSize;	
    }
	...
	if (Environment.MEDIA_MOUNTED.equals(Environment
	    ...
	    final String path = getExternalCacheDir(context, fileName)
		    .getPath();
	...
	// if Android sdk before froyo we need to construct cache dir by
	// ourselves
